...
const SessionsTab = () => {
  ...
  const [filter, setFilter] = useState<{
    ...
    values: { cinema: null, hall: null, date: null },
  ...
  const [deleteQuery, { isLoading: deleteLoading, error: deleteError }] =
    serverAPI.useDeleteSessionMutation();
  ...
  return (
    <Grid2 container px={{ md: "24px" }} columnSpacing={2}>
      ...
      <Grid2 size={{ xs: 12, md: 8 }}>
        <AdminTable
          ...
          values={(item) => [
            ...
            item.formatType,
            item.price,
            dayjs(item.startTime).format("ddd DD MMM"),
      ...
      <DeleteDialog
        ...
        onClose={() => setDeleteSession(undefined)}
        type="session"
        name={`Film: ${deleteSession?.filmName}\nCinema: ${
          deleteSession?.cinemaLocation
